...
void DistantLand::adjustFog() {
    ...
    if ((Configuration.MGEFlags & USE_DISTANT_LAND) && (Configuration.MGEFlags & USE_ATM_SCATTER) && mwBridge->CellHasWeather() && !mwBridge->IsUnderwater(eyePos.z)) {
        ...
        const float sunaltitude_c = (float)saturate(exp(-4.0 * sunPos.z)) * (float)saturate(sunaltitude);
        ...
        fogdist = (float)saturate(0.224 * fogdist);
        ...
        float mie = (1.58f / (1.24f - suncos)) * sunaltitude_c;
        float rayl = 1.0f - 0.09f * mie;
        float atmdep = 1.33f;

        D3DXVECTOR3 scatter;
        float scatterT = 0.5f * (1 + suncos);
        ...
        D3DXVECTOR3 att = atmdep * scatter * (sunaltitude_a + 0.7f * mie);
        ...
        D3DXVECTOR3 k1 = att * (1.17f * atmdep + 0.89f) * sunaltitude_b;
        c1.r = k0.x * k1.x;
        c1.g = k0.y * k1.y;
        c1.b = k0.z * k1.z;
...
void DistantLand::setSunLight(const D3DLIGHT9* s) {
